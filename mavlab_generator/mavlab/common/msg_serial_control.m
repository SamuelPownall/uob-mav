classdef msg_serial_control < mavlink_message
    ...
    properties        
		baudrate	%Baudrate of transfer. Zero means no change. (uint32[1])
		timeout	%Timeout for reply data in milliseconds (uint16[1])
		device	%See SERIAL_CONTROL_DEV enum (uint8[1])
		flags	%See SERIAL_CONTROL_FLAG enum (uint8[1])
		count	%how many bytes in this transfer (uint8[1])
