classdef msg_message_interval < MAVLinkMessage
	%MSG_MESSAGE_INTERVAL: MAVLink Message ID = 244
    %Description:
    %    This interface replaces DATA_STREAM
    %    Can also be constructed by using a MAVLinkPacket as the only argument
	%Arguments:
    %    interval_us(MAVLinkPacket): Alternative way to construct a message using a MAVLinkPacket
    ...
    methods(Static)
        ...
        function send(out,interval_us,message_id,varargin)

            if nargin == 2 + 1
                msg = msg_message_interval(interval_us,message_id,varargin);
            elseif nargin == 2
                msg = msg_message_interval(interval_us);
            else
    ...
    methods
        ...
        %MSG_MESSAGE_INTERVAL: Create a new message_interval message object
            ...
            if nargin == 1 
                if isa(interval_us,'MAVLinkPacket')
                    packet = interval_us;
                    obj.sysid = packet.sysid;
                    obj.compid = packet.compid;
                    obj.unpack(packet.payload);
                else
                    MAVLink.throwTypeError('interval_us','MAVLinkPacket');
                end
        ...
        %    for transmission.
            ...
            if errorField == 0
                ...
                packet = MAVLinkPacket(msg_message_interval.LEN);
