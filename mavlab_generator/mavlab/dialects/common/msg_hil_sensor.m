classdef msg_hil_sensor < MAVLinkMessage
	%MSG_HIL_SENSOR: MAVLink Message ID = 107
    %Description:
    ...
    %    xgyro(single): Angular speed around X axis in body frame (rad / sec)
    %    ygyro(single): Angular speed around Y axis in body frame (rad / sec)
    %    zgyro(single): Angular speed around Z axis in body frame (rad / sec)
    ...
    %    diff_pressure(single): Differential pressure (airspeed) in millibar
    %    pressure_alt(single): Altitude calculated from pressure
    %    temperature(single): Temperature in degrees celsius
    %    fields_updated(uint32): Bitmask for fields that have updated since last message, bit 0 = xacc, bit 12: temperature, bit 31: full reset of attitude/position/velocities/etc was performed in sim.
    ...
    methods(Static)
        ...
        function send(out,time_usec,xacc,yacc,zacc,xgyro,ygyro,zgyro,xmag,ymag,zmag,abs_pressure,diff_pressure,pressure_alt,temperature,fields_updated,varargin)
            ...
            if nargin == 15 + 1
                msg = msg_hil_sensor(time_usec,xacc,yacc,zacc,xgyro,ygyro,zgyro,xmag,ymag,zmag,abs_pressure,diff_pressure,pressure_alt,temperature,fields_updated,varargin);
            elseif nargin == 2
                msg = msg_hil_sensor(time_usec);
            else
    ...
    methods
        ...
        function obj = msg_hil_sensor(time_usec,xacc,yacc,zacc,xgyro,ygyro,zgyro,xmag,ymag,zmag,abs_pressure,diff_pressure,pressure_alt,temperature,fields_updated,varargin)
        %MSG_HIL_SENSOR: Create a new hil_sensor message object
        ...
        %    for transmission.
            ...
            if errorField == 0
                ...
                packet = MAVLinkPacket(msg_hil_sensor.LEN);
